...
const PillarImagePrev: FunctionComponent<PillarImagePrevProps> = ({ pillar }) => {
  const [isMouseOn, setIsMouseOn] = useState<Boolean>(false);

  return (
    <Tilt>
      <div
        className="flex cursor-pointer flex-col rounded-2xl justify-center items-center tajwal-font text-xl font-semibold pb-4 relative px-4 max-sm:!p-0"
        style={{ boxShadow: '#333333 0px 0px 37px -23px' }}
        onMouseEnter={() => setIsMouseOn(true)}
        onMouseLeave={() => setIsMouseOn(false)}
      >
        <img src={pillar.image} alt={pillar.caption} className="max-sm:w-20" />
